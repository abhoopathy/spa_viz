...
class Main
  ...
  def self.process_file(input,output)
  ...
  def self.process_dir(dir_name)
    puts "starting: #{dir_name}"

    output_dir_name = dir_name + "_processed"
    ...
    dir.collect{ |f| file_names.push f.gsub(/\.txt/,'') if f.include?(".txt") }

    file_names.each do |file_name|
      process_file( "#{dir_name}/#{file_name}.txt", "#{output_dir_name}/#{file_name}.xml" )
    end

    puts "completeted: #{dir_name}"
  end

  dir_names = ["Traci", "VOC", "Water Use", "so2"]
